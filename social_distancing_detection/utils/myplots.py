...
def plot_one_box(x, img, color=None, label=None, line_thickness=3, dist=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    # #distance
    # ppm = dimensions.pixels_per_metric(c1, c2)
    # print('\npixils_per_metric:',ppm)
    # dist = dimensions.img_distance(c1, c2)
    # dist = 74

    #Applying Social Distancing Detector
    for i in dist:   #remove duplicates
      if min(dist) < 12:
        dist.remove(i)

    min_dist = min(dist) #get closest bbox
    #print(min_dist)
    if min_dist <= 72:
      cv2.rectangle(img, c1, c2, color[0], tl, lineType=cv2.LINE_AA)
    else:
      cv2.rectangle(img, c1, c2, color[1], tl, lineType=cv2.LINE_AA)
    #Creating Label
    if label:
